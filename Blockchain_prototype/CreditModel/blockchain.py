...
class Blockchain:
    ...
    def check_credit_score(self):
        ...
        new_borrower = [[tx.sender == loan_sender for tx in block.transactions
                       if tx.recipient == participant] for block in self.__chain]        
        
        if loan_sender not in new_borrower:
            credit_score = 660
            return credit_score
            ...
            loan_duration = [[float(tx.duration) for tx in block.transactions
                     if tx.recipient == participant and
                     tx.sender == loan_sender] for block in self.__chain]
            ...
            credit_score = FicoScore.FicoScore.calculate_score(debt_ratio,payment_history,age_credit)
    ...
    def accept_loan(self, credit_score, amount, duration, apr):
        ...
        loan_transaction = Transaction('LOAN', self.hosting_node, '' , amount, duration, apr)
